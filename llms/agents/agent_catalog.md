...
- `audit_pr_elixir.md` (`audit-pr-elixir`): Staff-level PR reviewer for Elixir/Phoenix backends. Reviews correctness, design quality, security, performance, logging, and test coverage.
...
- `dev_backend_elixir_engineer.md` (`dev-backend-elixir-engineer`): Senior backend engineer for Elixir/Phoenix. Implements schemas, contexts, queries, background jobs, observability, and performance optimizations.
...
- `dev_frontend_ui_engineer.md` (`dev-frontend-ui-engineer`): Frontend engineer for Phoenix LiveView. Implements UI components, Tailwind styling, LiveView hooks, and responsive accessible interfaces.

- `dev_logging_daily_guardian.md` (`dev-logging-daily-guardian`): Logging quality guardian for day-to-day development. Reviews diffs for logging consistency, adds structured events, and normalizes metadata safely.
...
- `loc_i18n_ptbr_gettext_guardian.md` (`loc-i18n-ptbr-gettext-guardian`): i18n guardian for Brazilian Portuguese (pt-BR). Enforces Gettext usage, key naming conventions, domain organization, and translation quality.
...
- `qa_elixir_test_author.md` (`qa-elixir-test-author`): QA-driven Elixir test writer. Converts scenarios into ExUnit tests with proper test layers (unit, integration, LiveView, Oban), minimal fixtures, and actionable failures.
